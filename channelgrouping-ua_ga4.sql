CREATE OR REPLACE TABLE `federation-francaise-football.site_billetterie_dataset.channelgrouping_agregee_UA_GA4` AS
WITH ga4_events AS (
    SELECT
        PARSE_DATE('%Y%m%d', CAST(event_date AS STRING)) AS date,
        event_name,
        event_bundle_sequence_id AS session_id,
        traffic_source.source AS channelgrouping,  -- Ajout de la dimension channelgrouping
...
-- Comptage des événements session_start par date et channelgrouping pour GA4
...
-- Comptage des pages vues par date et channelgrouping pour GA4
ga4_pageviews AS (
    SELECT
        date,
        channelgrouping,
...
ga4_sessions_data AS (
    SELECT
        PARSE_DATE('%Y%m%d', CAST(event_date AS STRING)) AS date,
        event_bundle_sequence_id AS session_id,
        traffic_source.source AS channelgrouping,
    ...
    GROUP BY
        date, session_id, channelgrouping
),

-- Calcul du taux de rebond par date et channelgrouping pour GA4
...
ga4_final AS (
    SELECT
        sc.date AS date,
        sc.channelgrouping AS channelgrouping,
    ...
    LEFT JOIN
        ga4_pageviews pv
    ON
        sc.date = pv.date AND sc.channelgrouping = pv.channelgrouping
    LEFT JOIN
        ga4_bounce_rate_data brd
    ON
        sc.date = brd.date AND sc.channelgrouping = brd.channelgrouping
),
...
ua_final AS (
    SELECT
        ...
        'UA' AS source
    FROM
        `federation-francaise-football.site_billetterie_dataset.channelgrouping_historique_ua_billetterie`
)
